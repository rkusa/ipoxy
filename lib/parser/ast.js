...
Path.prototype.compile = function(locals) {
  ...
  for (var i = 0, len = locals.size; i < len; ++i) {
    ...
    if (local.has && local.has(path[0])) {
      obj = locals.get(i)
      break
    } else if (path[0] in local) {
      obj = local[path[0]]
      break
    }
  }
  ...
  if (Model.isModel(obj)) {
    root   = path.length ? obj.get().getIn(path) : obj.get()
    cursor = Cursor.from(obj.get(), path, function(newData) {
      obj.set(newData)
    })
  } else if (obj && typeof obj === 'object') {
    cursor = root = obj.getIn(path)
  } else {
    key = undefined
    cursor = root = obj
  }

  return new Reference(cursor, key, root)
}
...
Reference.prototype.get = function() {
  var result = this.key ? (this.obj && this.obj.get(this.key)) : this.obj
